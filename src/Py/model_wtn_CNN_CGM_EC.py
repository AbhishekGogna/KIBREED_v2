...
class tuner(kt.HyperModel):
    def __init__(self, marker_n, g_s_n, tune = False, batch_norm = False):
        self.marker_n = marker_n # markers
        self.g_s_n = g_s_n
        self.tune = tune
        self.batch_norm = batch_norm
        print(f'markers = {self.marker_n} and g_a_s\'s = {self.g_s_n}')

    def build(self, hp):
        # generate network for ec data
        net_g_s = CNN_net_fixed(hp, n_in = self.g_s_n, 
                               model_name = "g_s", 
                               kernel_size = 2)
